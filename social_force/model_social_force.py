...
class CrowdAgent(Agent):
    ...
    def __init__(self, unique_id, model):
        ...
        self.mass = 1
        # v = np.array(self.pos) - np.array(self.current_goal)
        # self.velocity = v/np.linalg.norm(v)
    ...
    def move_towards_goal2(self):
        ...
        if distances:
            ...
            if not near_fire:
                self.model.grid.move_agent(self, next_move) #move
    ...
    def total_force(self, state, t, f):
        x, self.velocity = state[:2], state[2:]
        dvdt = f/self.mass
        dxdt = self.velocity
        return np.concatenate((dxdt, dvdt))

    def ode_solver(self, prev_state, force):
        #ensure that both pos and velocity are numpy arrays
        prev_state = [np.array(prev_state[0]), np.array(prev_state[1])]

        # Initial state
        init_state = np.concatenate((prev_state[0], prev_state[1]))
        # Time points
        t = np.linspace(0, 1, 100)
        # Solve ODE
        sol = odeint(self.total_force, init_state, t, args=(force, ))

        sol_pos = sol[:, :2]
        sol_velocity = sol[:, 2:]
        return sol_pos, sol_velocity

    def calculate_f(self):
        return self.acceleration_force(0.5)

    def acceleration_force(self, tau = 0.5):
        #calculate vector between agent and goal
        desired_v = np.linalg.norm(np.array(self.pos) - np.array(self.current_goal)) #v^0, the norm of the vector between agent and goal

        #vector: difference between agent and goal
        v = np.array(self.pos) - np.array(self.current_goal)
        e0 = v/np.linalg.norm(v)

        return (desired_v*e0 - self.velocity)/tau
    ...
    def move_towards_goal(self):
        ...
        f = self.calculate_f()
        new_position, new_velocity = self.ode_solver([self.pos, self.velocity], f)

        # Update the agent's position and velocity
        self.velocity = new_velocity[-1]
        new_position = tuple(np.round(new_position[-1]).astype(int))

        # discretize new_position
        new_position = tuple(np.round(new_position[-1]).astype(int))
