...
def run_sensitivity_analysis(param_name, param_values, num_iterations=10):
    ...
    for value in tqdm(param_values, desc=f"Running sensitivity analysis on {param_name}"):
        for iteration in range(num_iterations):
            # Copy default parameters and set the one we're testing
            params = default_params.copy()
            params[param_name] = value
            ...
            results.append({
                ...
                "value": value,
                "iteration": iteration,
                "steps_to_completion": step_count
            })
...
if __name__ == "__main__":
    ...
    num_agents_values = [50, 100]
    goal_radius_values = [5, 10, 15, 20]

    # Run sensitivity analysis for each parameter
    results_num_agents = run_sensitivity_analysis("N", num_agents_values)
    results_goal_radius = run_sensitivity_analysis("goal_radius", goal_radius_values)

    # Combine and save results
    all_results = pd.concat([results_num_agents, results_goal_radius], ignore_index=True)
    all_results.to_csv("sensitivity_analysis_results.csv", index=False)