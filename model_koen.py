...
class CrowdAgent(Agent):
    ...
    def move_towards_goal(self):
        ...
        for neighbor in valid_neighbors:
            # print(neighbor)
            if neighbor == self.current_goal:
                goal_neighbors_coords = self.model.grid.get_neighborhood(neighbor, moore=True, include_center=False)
                # goal_neighbors = [neighbor for neighbor in goal_neighbors_coords if not self.model.grid.is_cell_empty(neighbor)]
                goal_neighbors = [agent for agent in self.model.schedule.agents if agent.pos in goal_neighbors_coords]

                agent_leaving = random.choice(goal_neighbors)
                # agent_leaving = [agent for agent in self.model.schedule.agents if agent.pos == agent_leaving_coords][0]
                # print(agent_leaving.pos)

                print(f"Agent {agent_leaving.unique_id} reached the goal!")
                self.model.grid.remove_agent(agent_leaving)
                self.model.schedule.remove(agent_leaving)

                # the others need to stand still
                list(goal_neighbors).remove(agent_leaving)
                # goal_neighbors = [agent for agent in self.model.schedule.agents if agent.pos in goal_neighbors]
                for agent in goal_neighbors:
                    agent.stand_still()

            elif self.model.grid.is_cell_empty(neighbor):
                distance = np.linalg.norm(np.array(neighbor) - np.array(self.current_goal))
